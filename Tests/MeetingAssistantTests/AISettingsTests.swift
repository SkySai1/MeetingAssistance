...
@Test func settingsAndPromptRoundTripThroughPrivateUserDirectory() throws {
    ...
    configuration.responsePreviewCharacters = 250
    ...
    #expect(!settings.contains("systemPrompt") && settings.contains("responsePreviewCharacters"))
...
@Test @MainActor func settingsMigrationPreservesCustomPromptAndReloadsNewLimits() throws {
    ...
    first.configuration.responsePreviewCharacters = 300
    first.configuration.factLimit = 4
    let restarted = AISettingsViewModel(preferences: preferences, store: store)
    #expect(restarted.configuration.responsePreviewCharacters == 300 && restarted.configuration.factLimit == 4)
